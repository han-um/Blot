...
contract BlotProject {
    ...
    // 프로젝트 정보 저장
    function createProject(string memory projectId, string memory writerId, string memory deadline, uint256 reward) public returns (bool) {
    ...
    // 프로젝트 보상금 조회
    function getProjectReward(string memory projectId) public view returns (uint256) {
    ...
    // 프로젝트 정보 조회
    function getProjectInfo(string memory projectId) public view returns (string memory, string memory, uint256) {
        require(projectExist(projectId), "ERROR : There is not the project. Can't get project info.");
        return (projectInfo[projectId].writerId, projectInfo[projectId].deadline, projectInfo[projectId].reward);
    }
