...
async function test() {
    ...
    .then(res => {
        ...
        var projScore = new Array();
        for(var i=0; i<projSize; i++) {
            projIdArray.push(res[i].projId);
            projScore[res[i].projId]=res[i].cnt;
        }
    ...
    db.Library.findAll({
        attributes: [
            'projId',
            [db.sequelize.fn('count', 'projId'), 'cnt']
        ],
        group:'projId'
    })
    .then(result => {
        var array = new Array();
        for(var i=0; i<result.length; i++)
            array.push({projId: result[i].projId, cnt: result[i].dataValues.cnt});
...
*/
    ...
    // 그에 해당하는 Project Id에 해당하는 정보 불러오기
}
