...
/* @ NOTE
  ...
  */
// 클라이언트로부터 프로젝트 이미지 파일이 post 방식으로 전송된 경우
router.post('/upload/project', upload.single('projectFile'), function(req, res) {
    if(req.file) {
        res.end(req.file.filename); 
    } else
        res.end('projDefault.jpg'); 
...
// 클라이언트로부터 사용자 이미지 파일이 post 방식으로 전송된 경우
router.post('/upload/user', upload.single('userFile'), function(req, res) {
    if(req.file) {
        res.end(req.file.filename); 
    } else
        res.end('userDefault.png'); 
});
